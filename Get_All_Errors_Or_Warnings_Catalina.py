...
def GET_LINES_INDEXES_GREP(log, string, time_grep):
    time_grep=datetime.strptime(time_grep,"%d-%m-%Y %H:%M:%S")
    indexes=[]
...
logs=[os.path.join(log_path,fil) for fil in logs if '.out' in fil]
...
for log in logs:
    ...
    for index in exception_indexes:
        try:
            key=log_data[index].split(string_for_grep)[-1]
            value=log_data[index+1].strip()
            print key
            print value
        ...
        if index not in printed_indexes:
            ...
            while x<=stop_ind:
                if 0<=x<len_log_data:
                    ...
                    if log_data[x].startswith('\t') and stop_ind+1 not in exception_indexes:
                        stop_ind+=1
                else:
                    f.write('\n')
                x+=1
